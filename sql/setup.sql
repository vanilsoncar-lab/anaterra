...
-- VERSAO 2: adiciona campo email na tabela duplas
...
-- ------------------------------------------------------------
-- Remove objetos existentes (para recriar limpo)
-- ------------------------------------------------------------
drop function if exists cadastrar_dupla(text,text,text,text) cascade;
drop function if exists cadastrar_dupla(text,text,text,text,text) cascade;
drop function if exists autenticar_dupla(text,text) cascade;
drop function if exists registrar_carta_e_arquetipo(uuid,integer,text) cascade;
drop function if exists registrar_resposta(uuid,integer,text,text) cascade;
drop function if exists autenticar_professor(text,text) cascade;
drop function if exists desvincular_carta(uuid) cascade;
drop table if exists conquistas cascade;
drop table if exists respostas cascade;
drop table if exists duplas cascade;
...
create table duplas (
  ...
  arquetipo text check (arquetipo in (
    'ana_terra','pedro_missioneiro','maneco_terra',
    'henriqueta','horacio','pedro_terra'
  )),
...
create policy "cartas leitura publica" on cartas for select using (true);
create policy "duplas leitura publica" on duplas for select using (true);
create policy "duplas sem insercao direta" on duplas for insert with check (false);
create policy "duplas sem update direto" on duplas for update using (false);
create policy "respostas leitura publica" on respostas for select using (true);
create policy "respostas sem insercao direta" on respostas for insert with check (false);
create policy "conquistas leitura publica" on conquistas for select using (true);
create policy "conquistas sem insercao direta" on conquistas for insert with check (false);
create policy "professores bloqueado" on professores for all using (false) with check (false);

-- ============================================================
-- FUNCOES RPC
-- ============================================================

-- cadastrar_dupla (com email)
...
declare
  v_id uuid;
begin
  insert into duplas (nome_dupla, integrantes, email, turma, senha_hash)
  values (
    p_nome_dupla,
    p_integrantes,
    lower(trim(p_email)),
...
$$;
grant execute on function cadastrar_dupla(text,text,text,text,text) to anon;

-- autenticar_dupla (por email + senha)
create or replace function autenticar_dupla(p_email text, p_senha text)
returns uuid
language plpgsql
security definer
as $$
...
begin
  ...
  if v_hash = crypt(p_senha, v_hash) then return v_id;
  else return null;
  end if;
end;
$$;
...
begin
  if p_carta_numero < 1 or p_carta_numero > 15 then return 'invalido'; end if;
  if p_arquetipo not in ('ana_terra','pedro_missioneiro','maneco_terra','henriqueta','horacio','pedro_terra') then return 'invalido'; end if;

  select carta_numero into v_ja_tem_carta from duplas where id = p_dupla_id;
  if v_ja_tem_carta is not null then return 'ja_registrado'; end if;

  select id into v_carta_existente from duplas
  where carta_numero = p_carta_numero and id <> p_dupla_id;
  if v_carta_existente is not null then return 'carta_ocupada'; end if;

  update duplas set carta_numero = p_carta_numero, arquetipo = p_arquetipo where id = p_dupla_id;
  return 'ok';
exception
  when unique_violation then return 'carta_ocupada';
end;
$$;
...
declare
  ...
  v_ja_existe integer;
begin
  select carta_numero into v_carta_dupla from duplas where id = p_dupla_id;
  if v_carta_dupla is null or v_carta_dupla <> p_carta_numero then return 'sem_permissao'; end if;

  select count(*) into v_ja_existe from respostas
  where dupla_id = p_dupla_id and carta_numero = p_carta_numero;
  if v_ja_existe > 0 then return 'ja_respondido'; end if;
  ...
  select xp_total + v_xp_ganho into v_novo_xp from duplas where id = p_dupla_id;
  v_novo_nivel := floor(v_novo_xp / 100) + 1;
  update duplas set xp_total = v_novo_xp, nivel = v_novo_nivel where id = p_dupla_id;

  insert into conquistas (dupla_id, codigo, titulo, descricao)
  values (p_dupla_id, 'primeira_estacao', 'Primeira estação concluída',
    'Você completou sua primeira missão na jornada de Ana Terra.')
...
declare
  v_hash text;
begin
  select senha_hash into v_hash from professores where nome = p_nome;
...
as $$
begin
  update duplas set carta_numero = null, arquetipo = null where id = p_dupla_id;
